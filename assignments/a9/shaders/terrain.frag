...
//fractal brownian noise - different noise implementation than perlin
// source: https://www.shadertoy.com/view/4dS3Wd
//this is our implementation of the hash function for fractal brownian noise
float random (in vec2 st) {
    return fract(sin(dot(st.xy, vec2(1,2.233))));
}

float fractal_brownian_noise (in vec2 st) {
    vec2 i = floor(st);
    vec2 f = fract(st);
...
float noiseOctave(vec2 v, int num)
{
	float sum = 0;
	for(int i =0; i<num; i++){
		sum += pow(2,-1*i) * fractal_brownian_noise(pow(2,i) * v);
	}
	return sum;
}
...
vec4 shading_phong(light li, vec3 e, vec3 p, vec3 s, vec3 n) 
{
    ...
    vec3 difColor = kd * li.dif.rgb * max(0., dot(n, l)) * pow(length(s - p), 2);
    vec3 specColor = ks * li.spec.rgb * pow(max(dot(v, r), 1.0049), shininess);
...
vec3 shading_terrain(vec3 pos) {
	...
	vec3 emissiveColor = mix(vec3(1.0, 1.0, 1.0), vec3(1., 1., 1.), h);
